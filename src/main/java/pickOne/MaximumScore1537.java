        ...
        if (nums1[nums1.length - 1] < nums2[0]) {
            ...
            while (fr < nums1.length && sr < nums2.length) {
                if (nums1[fr] == nums2[sr]) {
                    res = (res + Math.max(sum(nums1, fl, fr), sum(nums2, sl, sr))) % MOD_VALUE ;
                    fl = fr;
                    sl = sr;
                    sr++;
                    fr++;
            ...
            res = (res + Math.max(sum(nums1, fl, nums1.length), sum(nums2, sl, nums2.length))) % MOD_VALUE;
        }
